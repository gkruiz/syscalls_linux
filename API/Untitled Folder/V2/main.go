...
func info_process(w http.ResponseWriter, r *http.Request) {
	...
	histProcesos := make([]ElmentoArrR, 0, 11)

	for i := 0; i < len(valores); i++ {
		procesos := *(valores[i].Data)
		copia := make([]ProcInfoR, len(procesos))
		copy(copia, procesos)
		r := ElmentoArrR{
			Data: copia,
		}

		histProcesos = append(histProcesos, r)
	}

	println(len(histProcesos))
	println(len(valores))

	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(histProcesos)
}
