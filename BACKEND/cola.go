...
/*ESTA ES LA COLA PARA datos globales */

type FixedQueueG struct {
	data []CM_global
	cap  int
	mu   sync.Mutex
}

type CM_global struct {
	CPU uint64
	RAM uint64
	mu  sync.Mutex
}

func NewFixedQueueG(cap int) *FixedQueueG {
	return &FixedQueueG{
		data: make([]CM_global, 0, cap),
		cap:  cap,
	}
}

func (q *FixedQueueG) Enqueue(value CM_global) {
	q.mu.Lock()
	defer q.mu.Unlock()
...
func (q *FixedQueueG) Values() []CM_global {
	q.mu.Lock()
	defer q.mu.Unlock()

	// Devolver una copia para evitar condiciones de carrera externas
	result := make([]CM_global, len(q.data))
	copy(result, q.data)
	return result
}
