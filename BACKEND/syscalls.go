...
const SYS_SYSINFO_USAGE = 554

func GetGlobalInfo() (GenInfo, error) {
	var cpuUsage uint32
	var ramUsage uint32
	...
	_, _, errno := syscall.Syscall(
		...
		uintptr(unsafe.Pointer(&ramUsage)),
		0,
	)

	if errno != 0 {
		return GenInfo{}, errno
	}
...
const SYS_SYSINFO_USAGE = 548 // reemplazalo por el número correcto
...
func main() {
	...
	if errno != 0 {
		fmt.Println("Error al llamar a la syscall:", errno)
		return
	}
